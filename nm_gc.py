...
def f(x):
  ...
  NR1 = commands.getoutput("awk '{if($1<"+str(rhoin)+"){printf "%d", $2}}' F.plt | head -1")
  NR2 = commands.getoutput("awk '{if($1<"+str(x[2])+"){printf "%d", $2}}' F.plt | head -1")
  NR3 = commands.getoutput("awk '{if($1<"+str(rhoout)+"){printf "%d", $2}}' F.plt | head -1")
