...
def f(x):
  ...
  NR1 = commands.getoutput("awk '{if($1<"+str(rhoin)+"){printf "%d", NR}}' F.plt | head -1")
  NR2 = commands.getoutput("awk '{if($1<"+str(x[2])+"){printf "%d", NR}}' F.plt | head -1")
  NR3 = commands.getoutput("awk '{if($1<"+str(rhoout)+"){printf "%d", NR}}' F.plt | head -1")
  rhoin1  = commands.getoutput("awk '{if(NR=="+str(NR1)+"-2){printf "%12.5f",$2}}' F.plt")
  rhoin2  = commands.getoutput("awk '{if(NR=="+str(NR1)+"+2){printf "%12.5f",$2}}' F.plt")
  rhoe1   = commands.getoutput("awk '{if(NR=="+str(NR2)+"-2){printf "%12.5f",$2}}' F.plt")
  rhoe2   = commands.getoutput("awk '{if(NR=="+str(NR2)+"+2){printf "%12.5f",$2}}' F.plt")
  rhoout1 = commands.getoutput("awk '{if(NR=="+str(NR3)+"-2){printf "%12.5f",$2}}' F.plt")
  rhoout2 = commands.getoutput("awk '{if(NR=="+str(NR3)+"+2){printf "%12.5f",$2}}' F.plt")
